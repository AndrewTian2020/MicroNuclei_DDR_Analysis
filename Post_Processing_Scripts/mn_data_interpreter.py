...
# get_nuclei_scores returns a list of lists, one list per image
# each individual list contains scores for all nuclei in an image
def get_nuclei_scores(path_to_json):
    # a and b are used to cut the nuclei data down to the scores
    # c is used to parse every individual score (float)
    # n temporarily stores every individual score
    a = 0
    b = 0
    c = 0
    n = 0

    # nuclei_texts contains all the nuclei info
    # nuclei_score_text contains all the score text
    # score_text_list contains scores for each image
    # tempstring is used to temporarily store an image's score text
    # templist is used to store the scores for an individual image
    # nuclei_scores contains the scores for all images
    nuclei_texts = get_nuclei_info(path_to_json)
    score_text_list = []
    nuclei_scores = []

    # parse the text for the scores
    for text in nuclei_texts:
        score_text_list.append(parse_text(text, '"score"', '"mask"', 9, 2))

    # parse the score text for score values (floats)
    # and add those to a temporary list for each image
    # then add each image's list to the final list
    for item in score_text_list:
        nuclei_scores.append(get_float_list(item))

    # return the list of nuclei scores
    return nuclei_scores

# get_micronuclei_scores returns a list of lists, one list per image
# each individual list contains scores for all micronuclei in an image
def get_micronuclei_scores(path_to_json):
    # a and b are used to cut the micronuclei data down to the scores
    # c is used to parse every individual score (float)
    # n temporarily stores every individual score
    a = 0
    b = 0
    ...
    # micronuclei_texts contains all the micronuclei info
    # micronuclei_score_text contains all the score text
    # score_text_list contains scores for each image
    # tempstring is used to temporarily store an image's score text
    # templist is used to store the scores for an individual image
    # micronuclei_scores contains the scores for all images
    ...
    # then add each image's list to the final list
    for item in score_text_list:
        micronuclei_scores.append(get_float_list(item))

    # return the list of micronuclei scores
    return micronuclei_scores
...
# get_nuclei_boxes returns a list of box coordinates for each image's
# nuclei; the total list contains all images in a .json file
def get_nuclei_boxes(path_to_json):
    nuclei_texts = get_nuclei_info(path_to_json)
    nuclei_box_texts = []
    nuclei_boxes = []

    for text in nuclei_texts:
        nuclei_box_texts.append(parse_text(text, '"bbox"', '"score"', 0, 0))
    
    for item in nuclei_box_texts:
        nuclei_boxes.append(get_coords_list(item, 2))
    
    return(nuclei_boxes)

# get_micronuclei_boxes returns a list of box coordinates for each image's
# micronuclei; the total list contains all images in a .json file
def get_micronuclei_boxes(path_to_json):
    micronuclei_texts = get_micronuclei_info(path_to_json)
    micronuclei_box_texts = []
    micronuclei_boxes = []

    for text in micronuclei_texts:
       micronuclei_box_texts.append(parse_text(text, '"bbox"', '"score"', 0, 0))
    
    for item in micronuclei_box_texts:
        micronuclei_boxes.append(get_coords_list(item, 0))
    
    return(micronuclei_boxes)
...
class jsonFile:
    ...
    def __init__(self, path_to_json):
        ...
        self.nuclei_scores = get_nuclei_scores(path_to_json)
        self.micronuclei_scores = get_micronuclei_scores(path_to_json)
        self.mn_ratios = get_mn_ratios(path_to_json)
        self.nuclei_boxes = get_nuclei_boxes(path_to_json)
        self.micronuclei_boxes = get_micronuclei_boxes(path_to_json)
    ...
    # match_nuclei_scores matches all the nuclei center coordinates
    # to their respective rating for a given image in a jsonFile object
    def match_nuclei_scores(self, image_name):
        # n is the position of the image ID in the jsonFile object image ID list
        # i is used as an index for the while loop to merge the lists
        n = 0
        i = 0

        # templist temporarily stores a coordinate and its corresponding rating in a list
        # output is the final list to be outputted which stores an image's nuclei info + ratings
        # scores is the list of nuclei scores for the original image
        # coords is the list of nuclei center coordinates for the original image
        templist = []
        output = []
        scores = []
        coords = []

        n = self.image_ids.index(image_name)

        # check if the image is actually in the jsonFile list, if not then return an error
        if n != -1:
            scores = self.nuclei_scores[n]
            coords = self.nuclei_coords[n]
            # while loop continually merges the two lists together
            while i < len(coords):
                templist = [coords[i], scores[i]]
    ...
    # match_micronuclei_scores matches all the micronuclei center coordinates
    # to their respective rating for a given image in a jsonFile object
    def match_micronuclei_scores(self, image_name):
        # n is the position of the image ID in the jsonFile object image ID list
        # i is used as an index for the while loop to merge the lists
        n = 0
        i = 0
        ...
        # templist temporarily stores a coordinate and its corresponding rating in a list
        # output is the final list to be outputted which stores an image's micronuclei info + ratings
        # scores is the list of micronuclei scores for the original image
        # coords is the list of micronuclei center coordinates for the original image
        templist = []
        output = []
        scores = []
        coords = []
        ...
        # check if the image is actually in the jsonFile list, if not then return an error
        if n != -1:
            scores = self.micronuclei_scores[n]
            coords = self.micronuclei_coords[n]
            # while loop continually merges the two lists together
            while i < len(coords):
                templist = [coords[i], scores[i]]
                output.append(templist)
    ...
    def update_micronuclei(self, i, remove_indexes):
        ...
        self.update_list(self.micronuclei_boxes, i = i, remove_indexes = remove_indexes)
        self.update_list(self.micronuclei_scores, i = i, remove_indexes = remove_indexes)
...
#test = jsonFile("/Users/andrewtian/Documents/HardingLab/Andrews_Code/NIR_E.json")

#print(test.nuclei_counts)
#print(test.micronuclei_counts)
#print(test.mn_ratios)
#print("\n")

#test.apop_threshold(2)
#test.dist_threshold(25)
